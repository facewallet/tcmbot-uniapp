...
from dss.util.medical_embedding_util import do_embedding_4_user_slot_list, do_embedding_4_user_slot
from dss.util.snowflake_util import SnowflakeIdWorker, SnowflakeIDGenerator, ShortSnowflakeIDGenerator
...
# worker = SnowflakeIDGenerator(datacenter_id=1, worker_id=2)
worker = ShortSnowflakeIDGenerator(worker_id=1)
...
async def do_dialogue(dialogue: Dialogue):
    ...
    try:
        if dialogue.speaker == USER:
            if USER_ASK == dialogue.user_intent and dialogue.bot_intent is None:
                ...
                if results:
                    ...
                    for result in results:
                        ...
                        if result.need_fill == 'YES' and result.fill_method == 'boolean':
                            ...
                            #todo3 因为有些槽位不是必须的
                            radio_form.required = result.required
                            radio_form.type = RADIO
                            radio_options = []
                ...
                for edge_form in forms:
                    slot_id = edge_form.id
                    slot_name = edge_form.slot_name
                    slot_value = edge_form.value
                    # if edge_form.type == 'radio':
                    #     #todo 遍历options ，如果 option.value == slot_value，则将option.slot_name的值赋值给slot_name
                    #     for option in edge_form.options:
                    #         if option.value == slot_value:
                    ...
                    # todo 先查询有没有session_id,name,value的，如果没有则插入
                    user_slot_tmp = user_slot_dao.find_by_condition(session_id=session_id, name=slot_name, value=slot_value)
                    if user_slot_tmp is None or user_slot_tmp == []:
                        #todo3 do_embedding 完成
                        user_slot = do_embedding_4_user_slot(user_slot)
                        user_slot_dao.add(user_slot)


                    # 判断 slot_value 和 DecisionSlotMapping 中的 node_slot_value 是否一致
                    if slot_value == edge_form.slot_value:
                        # 获取当前的 DecisionSlotMapping 对象
                        slot_mapping_dao = DecisionSlotMappingDAO()
                        slot_mapping = slot_mapping_dao.find_by_condition(session_id=session_id, tree_id=tree_id,
                                                                          node_id=node_id, node_slot_id=slot_id)
                        if slot_mapping is not None or slot_mapping != []:
                            #todo3 do_embedding 完成
                            # 一致则更新 DecisionSlotMapping 中的用户字段和 flag
                            slot_mapping_tmp = slot_mapping[0]
                            slot_mapping_tmp.user_slot_name = slot_name
                            slot_mapping_tmp.user_slot_value = slot_value
                            slot_mapping_tmp.user_slot_embedding = user_slot.embedding
                            slot_mapping_tmp.flag = 'match'
                            slot_mapping_dao.update(slot_mapping_tmp)
                        else:
                            return fail_message
                    else:
                        # 不一致则增加冲突计数并记录冲突的 slot_id
                        # todo3 如果是必须的，才算冲突 完成
                        if edge_form.required == 'YES':
                            conflict_num += 1
                ...
                edge_form = forms[0]
                slot_name = edge_form.slot_name
                slot_value = edge_form.slot_value
                # todo 遍历edge_form中的options，如果跟edge_form中的value相等，则将slot_name和slot_value赋值给slot_name和slot_value
                for option in edge_form.options:
                    if option.value == edge_form.value:
                        slot_name = option.slot_name
                        slot_value = option.slot_value

                new_node_id = edge_form.value # todo 获取当前节点 这个地方需要再排查
                session_id = dialogue.session_id
                tree_id = dialogue.tree_id
                # todo 保存到user_slot表中
                user_slot_dao = DecisionUserSlotDAO()
