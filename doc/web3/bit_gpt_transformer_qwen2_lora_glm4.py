...
# 数据预处理
def preprocess_data(file_path, feature_range=(0, 1)):
    df = pd.read_csv(file_path)
    df = df[['Open', 'High', 'Low', 'Close', 'Volume_(BTC)']]

    scaler_price = MinMaxScaler(feature_range=feature_range)
    scaler_volume = MinMaxScaler(feature_range=feature_range)
...
class BitGPT(nn.Module):
    def __init__(self, dim_size=4096):
        ...
        self.transformer = Qwen2ForCausalLM.from_pretrained('qwen2', return_dict=False)
        for param in self.transformer.parameters():
            param.requires_grad = False
        self.lora_a = nn.Linear(dim_size, 1, bias=False)
        self.lora_b = nn.Linear(1, dim_size, bias=False)
...
# 训练模型
def train_model(model, X_tensor, y_tensor, epochs=100):
